  ...
  Future<void> fetchWireTransferSettings() async {
    try {
      ...
      if (response.status == Status.completed && response.data != null) {
        debugPrint('Wire transfer RAW: ${response.data}');
        wireTransferSettingsModel.value = WireTransferSettingsModel.fromJson(
          response.data!,
        );
        debugPrint('Wire transfer data loaded: ${wireTransferSettingsModel.value.data?.id}');
      } else {
        debugPrint('Wire transfer settings failed: ${response.status} ${response.data}');
      }
    } catch (e, st) {
      debugPrint('Wire transfer settings error: $e');
      debugPrint('Stack: $st');
    }
  }
      ...
      if (response.status == Status.completed) {
        ...
        Get.find<NavigationController>().pushPage(
          DynamicFundTransferSuccess(
            ...
            currency: "N/A",
            title: "Initiate Transfer Successfully!",
